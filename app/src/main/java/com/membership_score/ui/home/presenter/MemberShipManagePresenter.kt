...
class MemberShipManagePresenter : BasePresenter<HomeFragmentModel, MemberShipManageContract.IView>(),
    ...
    override fun insertOneMemberShipOperation(db: SQLiteDatabase?, info: MemberShipOperationBean?) {
        try {
            val rowid = SQLManger.getIntance().insertOneMemberShipOperation(db, info)
            mView.hideLoading()
            if (rowid >-1) {
                mView.insertOneMemberShipOperationSuccess()
            } else {
                mView.insertOneMemberShipOperationFaild("")
            }
        } catch (ex: Exception) {
            mView.insertOneMemberShipOperationFaild(ex.message)
        }
        db?.close()
    }
